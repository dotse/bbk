/*
 * $Id: tpio_unix.c,v 1.11 2005/08/17 11:12:52 rlonn Exp $
 * $Source: /cvsroot-fuse/tptest/os-dep/unix/tpio_unix.c,v $
 *
...
int AcceptClient(TPEngine *engp, int sockInx) {
       WSInfo * w;
		...
		w->tcpDataSock = s;
		engp->tcpDataConnectComplete = 1;
                engp->hostTCPDataPort = ntohs(clientSa.sin_port);
		w->dataConnOK = 1;
...
#endif
	for (i=engp->local_udp_pr_start; i<=engp->local_udp_pr_end; i++) {
		rcvSa.sin_family = AF_INET;
		rcvSa.sin_addr.s_addr = engp->myLocalAddress.s_addr;
		if (engp->myUDPRecvPort == 0)
		  rcvSa.sin_port = htons(engp->myUDPRecvPort);
		else
		  rcvSa.sin_port = htons(i);
		if( bind( w->udpRecvSock, (struct sockaddr *)(&rcvSa), sizeof rcvSa ) != 0 ) {
			if (i >= engp->local_udp_pr_end)
				goto fail;
		}
		else
			break;
...
// 		MessageBox(CurDlg, "Can't get local port number on rcv data socket", TPERROR, MB_OK);
	...
	engp->boundUDPRecvPort = ntohs(((SOCKADDR_IN *)&rcvSa)->sin_port);
	...
	for (i=engp->local_udp_pr_start; i<=engp->local_udp_pr_end; i++) {
		rcvSa.sin_family = AF_INET;
		rcvSa.sin_addr.s_addr = engp->myLocalAddress.s_addr;
		if (engp->myUDPSendPort == 0)
		  rcvSa.sin_port = htons(engp->myUDPSendPort);
		else
		  rcvSa.sin_port = htons(i);
...
// 		MessageBox(CurDlg, "Can't get local port number on rcv data socket", TPERROR, MB_OK);
	...
	engp->boundUDPSendPort = ntohs(((SOCKADDR_IN *)&rcvSa)->sin_port);

	printf("myUDPSendPort = %d      myUDPRecvPort=%d\n", 
		engp->boundUDPSendPort, engp->boundUDPRecvPort);
...
int InitTCPSock(TPEngine *engp, int sockInx) {
	...
	case TP_SOCKINX_CTRL:		// Client opening a control socket
		...
		engp->boundTCPControlPort = ntohs(locSa.sin_port);
	...
	case TP_SOCKINX_DATA:		// Client opening a data socket
		...
		for (i=engp->local_tcp_pr_start; i<=engp->local_tcp_pr_end; i++) {
			locSa.sin_family = AF_INET;
			locSa.sin_addr.s_addr = engp->myLocalAddress.s_addr;
			if (engp->myTCPDataPort != 0)
			  locSa.sin_port = htons(engp->myTCPDataPort);
			else 
			  locSa.sin_port = htons(i);
			if( bind( w->tcpDataSock, (struct sockaddr *)(&locSa), sizeof locSa ) != 0 ) {
				if (i >= engp->local_tcp_pr_end)
					goto fail;
			}
			else
				break;
		...
		// Get bound address/port combination
		if (getsockname(w->tcpDataSock, (SOCKADDR *)(&locSa), &saLen) == SOCKET_ERROR)
			goto fail;
		engp->myLocalAddress.s_addr = locSa.sin_addr.s_addr;
		engp->boundTCPDataPort = ntohs(locSa.sin_port);
	...
	case TP_SOCKINX_SCTRL:		// Server opening a listen control socket
		...
		// REUSEADDR so the OS will let us reuse that particular port next time
...
*/		
		...
		for (i=engp->local_tcp_pr_start; i<=engp->local_tcp_pr_end; i++) {
			...
			if (engp->myTCPDataPort != 0)
			  ...
			  locSa.sin_port = htons(i);
			if( bind( w->tcpServerDataSock, (struct sockaddr *)(&locSa), sizeof locSa ) != 0 ) {
				if (i >= engp->local_tcp_pr_end)
					goto fail;
			}
			else
