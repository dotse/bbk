/*
 * $Id: client.c,v 1.7 2005/08/17 11:12:36 rlonn Exp $
 * $Source: /cvsroot-fuse/tptest/apps/unix/client/client.c,v $
 *
...
void usage(char *s) {
  ...
  printf("  udp-send-auto | usa       UDP auto send to server\n");
  printf("    parameters: <start-bitrate>\n");
  printf("  udp-receive-auto | ura    UDP auto receive from server\n");
  printf("    parameters: <start-bitrate>\n");
  printf("\n");
  if (strlen(s)) {
    printf("%s\n", s);
...
int main(int argc, char **argv) {
  ...
  while ((ch = getopt(argc, argv, "m:b:v:s:n:e:p:d:S:R:t")) != -1) {
    switch (ch) {
      case 'm':
        ...
        else if (strcasecmp(optarg, "udp-receive-auto")==0 || strcasecmp(optarg, "ura")==0)
          selectedMode = CLM_AUTO_UDP_RECV;
        else if (strcasecmp(optarg, "udp-send-auto")==0 || strcasecmp(optarg, "usa")==0)
          selectedMode = CLM_AUTO_UDP_SEND;
  ...
  switch (selectedMode) {
	...
	strncpy(engp->hostName, argv[3], TP_HOST_NAME_SIZE);
      ...
      sprintf(output_text, "Test parameters\n"
		"\tTime: %ld   Packetsize: %ld   PPS: %ld   Bitrate: %ld\n",
		engp->sessionTime, engp->packetSize, engp->packetsPerSecond,
		engp->bitsPerSecond);
      log_text(2);
      break;
    case CLM_TCP_SEND:
    ...
    case CLM_AUTO_UDP_RECV:
      if (argc == 3) {
	engp->flags |= TPENGINE_FLAGS_NOINITRATE;
	engp->bitsPerSecond = atoi(argv[0]);
        RecalculatePPSSZ(engp);
        strncpy(engp->hostName, argv[1], TP_HOST_NAME_SIZE);
  ...
  while (1) {
    ...
    /* perform more tests */
    engp->tpMode = CLM_NONE;
    engp->bitsPerSecond = 0;
    engp->packetsPerSecond = 0;
    engp->packetSize = 0;
...
void ReportResults(int selectedMode, TPEngine *engp, time_t * starttime, time_t * stoptime) {
  ...
  if (selectedMode == CLM_AUTO_UDP_SEND) {
    sprintf(output_text, "UDP Send: %d bps (%s)\n",
      (int)(engp->bestUDPSendRate * 8.0), Int32ToString((int)(engp->bestUDPSendRate * 8.0)));
    log_text(0);
  }
  if (selectedMode == CLM_AUTO_UDP_RECV) {
    sprintf(output_text, "UDP Recv: %d bps (%s)\n",
      (int)(engp->bestUDPRecvRate * 8.0), Int32ToString((int)(engp->bestUDPRecvRate * 8.0)));
    log_text(0);
  }
